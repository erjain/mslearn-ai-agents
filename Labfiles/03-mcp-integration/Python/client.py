...
async def connect_to_server(exit_stack: AsyncExitStack):
    ...
    # Start the MCP server process and create a client session connected to it
    stdio_transport = await exit_stack.enter_async_context(stdio_client(server_params))
    stdio, write = stdio_transport

    # Create an MCP client session connected to the server process via stdio
...
async def chat_loop(session):
    ...
    with (
        ...
        # Build a function for each tool that the agent can call to execute the tool via the MCP client session
        ...
        # Create FunctionTool definitions for the agent based on the tools available from the MCP server
